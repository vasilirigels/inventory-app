...
function HurdaList({ date, onOpen, onCreate, onDelete, refreshKey }) {
  ...
  useEffect(() => {
    ...
    const url = fromDate === toDate
      ? `/api/hurda-purchases/by-date/${fromDate}`
      : `/api/hurda-purchases/by-range?from=${fromDate}&to=${toDate}`
    fetch(url)
      .then(r => r.json())
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-slate-800">🟡 Konvertim Hurda</h2>
          <p className="text-xs text-slate-500">
            Blerje flori i përdorur me gram. Shuma e paguar zbritet nga arka. Floriri qëndron aty
            si stok hurde (nuk kalon në inventarin e produkteve).
          </p>
        </div>
        <button onClick={onCreate} className="btn-primary">+ Blerje Hurde e Re</button>
      </div>
        ...
        {loading ? (
          ...
          <div className="p-10 text-center">
            ...
            <p className="text-slate-500 mb-4">
              {fromDate === toDate
                ? 'Nuk ka blerje hurde për këtë datë.'
                : 'Nuk ka blerje hurde në këtë periudhë.'}
            </p>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase">Gram</th>
...
function HurdaEditor({ date, purchaseId, onClose, onSaved }) {
  const [loading, setLoading]     = useState(true)
  ...
  const save = async () => {
    if (saving) return
    if (n(gram) <= 0) { alert('Vendos sasinë në gram.'); return }
    ...
    try {
      const payload = {
        ...
        gram: n(gram),
        price_per_gram: n(pricePerGram), // EUR/gram
        currency: 'EUR',
        exchange_rate: n(exchangeRate) || 1, // 1 EUR = ? LEK (për raportin në LEK)
        total_amount: totalEur,              // gjithmonë në EUR
        notes,
      }
      const url = purchaseId ? `/api/hurda-purchases/${purchaseId}` : '/api/hurda-purchases'
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-800">
              {purchaseId ? 'Edito Blerjen e Hurdës' : 'Blerje Hurde e Re'}
            </h2>
      ...
      <div className="card grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <input type="number" step="0.001" min="0"
            ...
            className="input-field tabular-nums font-semibold text-amber-700" placeholder="0.00" />
        </div>
        <div>
      ...
      <div className="card bg-amber-50 border-amber-200">
        <p className="text-xs text-amber-800">
          ...
          Floriri prej <b>{fmtGram(gram || 0)} gram</b> qëndron si stok hurde — nuk krijohet produkt në inventar.
        </p>
      </div>
    </div>
  )
...
export default function KonvertimHurda({ date }) {
  ...
  if (mode === 'edit') {
    return <HurdaEditor date={date} purchaseId={editingId} onClose={backToList} onSaved={onSaved} />
  }
  return (
    <HurdaList
      date={date}
