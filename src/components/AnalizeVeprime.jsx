...
function exportActivityExcel(enriched, totals, client, dateRange, currencyByInvoiceId) {
  const rows = enriched.map(ev => {
    ...
    return {
      ...
      'Monedha': currencyByInvoiceId?.get(p.invoice_id) || '',
...
function exportActivityPdf(enriched, totals, client, dateRange, currencyByInvoiceId) {
  ...
  exportToPdf(`Analize Veprime Klient${client?.name ? ` — ${client.name}` : ''}`, [
    {
      subtitle,
      headers: ['Data', 'Lloji', 'Burimi', 'Detaje', 'Monedha', 'Pagesa', 'Debitim (+)', 'Kreditim (−)', 'Balanca'],
      rows: enriched.map(ev => {
        ...
        return [
          ...
          currencyByInvoiceId?.get(p.invoice_id) || '',
...
function pmBadge(pm) {
  if (pm === 'debt') return <span className="badge bg-amber-100 text-amber-700 dark:text-amber-300">⚠️ Borxh</span>
  if (pm === 'bank') return <span className="badge bg-blue-100 text-blue-700 dark:text-blue-300">🏦 Bankë</span>
  if (pm === 'pos')  return <span className="badge bg-purple-100 text-purple-700 dark:text-purple-300">💳 POS</span>
  return <span className="badge bg-emerald-100 text-emerald-700 dark:text-emerald-300">💵 Cash</span>
}
  ...
  return (
    <div className="card p-0 overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
        ...
        <p className="text-[10px] text-slate-500 dark:text-slate-400">{rows.length} klientë me fatura</p>
      </div>
      {rows.length === 0 ? (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="text-right">
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{r.invoice_count}f.</p>
...
function ActivityPanel({ client, onNavigate, dateRange }) {
  ...
  if (!client) {
    return (
      <div className="card text-center py-16">
        ...
        <p className="text-slate-500 dark:text-slate-400">Zgjidh një klient nga lista majtas, ose kërko sipër.</p>
      </div>
    )
  }
  ...
  if (!data || data.invoices.length === 0) {
    return (
      <div className="card text-center py-12">
        ...
        <p className="text-slate-500 dark:text-slate-400">Nuk ka asnjë faturë për këtë klient.</p>
      </div>
    )
  }
  ...
  const enriched = events.map(ev => {
    if (ev.kind === 'invoice') {
      ...
      const cur  = currencyByInvoiceId.get(ev.payment.invoice_id) || 'LEK'
        ...
        <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
              ...
              <span className="ml-1 mr-1 badge bg-blue-100 text-blue-700 dark:text-blue-300">📄 FATURA SHITJE</span>
              · Pagesat e mëvonshme nga
              <span className="ml-1 badge bg-emerald-100 text-emerald-700 dark:text-emerald-300">💰 Detyrime Klienti</span>
            </p>
          </div>
          <div className="flex gap-2 flex-shrink-0">
            <button
              onClick={() => exportActivityExcel(enriched, totals, client, dateRange, currencyByInvoiceId)}
              className="btn-secondary text-xs"
              title="Eksporto në Excel"
            >📊 Excel</button>
            <button
              onClick={() => exportActivityPdf(enriched, totals, client, dateRange, currencyByInvoiceId)}
              className="btn-secondary text-xs"
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
            <tr>
              ...
              <th className="px-4 py-3 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Debitim (+)</th>
              <th className="px-4 py-3 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Kreditim (−)</th>
          ...
          <tbody>
            {enriched.map((ev, i) => {
              if (ev.kind === 'invoice') {
                ...
                return (
                  <tr key={i} className={`border-b border-slate-100 dark:border-slate-800 ${isCredit ? 'bg-red-50/40 hover:bg-red-50' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                    <td className="px-4 py-2 text-slate-700 dark:text-slate-200 font-semibold">{ev.date}</td>
                    <td className="px-4 py-2">
                      {isCredit
                        ? <span className="badge bg-red-100 text-red-700 dark:text-red-300">↩️ Kreditore</span>
                        : <span className="badge bg-blue-100 text-blue-700 dark:text-blue-300">📄 Faturë Shitje</span>}
