    ...
    <div className="space-y-4">
      <div className="card">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">🏷️ Produkte në Promocion</h2>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex items-center justify-between">
          ...
          {rows.length > 0 && isAdmin && (
            <span className="text-[10px] text-slate-400 dark:text-slate-500">Ndrysho vlerat dhe kliko 💾 për të ruajtur</span>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-slate-400 dark:text-slate-500 text-sm">Asgjë nuk përputhet me kërkimin.</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left  text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">SKU / Barkod</th>
                  <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase w-32">Çmimi Origjinal</th>
                  ...
                  <th className="px-3 py-2 text-right text-xs font-semibold text-emerald-700 dark:text-emerald-300 uppercase" title="Çmimi pas aplikimit të zbritjes së promocionit">Çmimi me Promo</th>
                  <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Stok</th>
                  <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase w-28"></th>
                </tr>
              </thead>
              <tbody>
                {visible.map(p => {
                  ...
                  const fl = flash[p.id]
                  return (
                  <tr key={p.id} className={`border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 ${dirty ? 'bg-amber-50/40' : ''}`}>
                    ...
                    <td className="px-3 py-2 font-mono text-[11px] text-slate-500 dark:text-slate-400">
                      {p.sku || '—'}
                      {p.barcode && <div className="text-slate-400 dark:text-slate-500">{p.barcode}</div>}
                    </td>
                    <td className="px-3 py-2">
                      {isAdmin ? (
                        <MoneyInput
                        ...
                        <span className="text-right tabular-nums text-slate-500 dark:text-slate-400 line-through">€{fmt(editedSell)}</span>
                      )}
                    </td>
                    <td className="px-3 py-2">
                      {isAdmin ? (
                    ...
                    <td className="px-3 py-2 text-right tabular-nums font-bold text-emerald-700 dark:text-emerald-300">
                      €{fmt(promoPrice)}
                    </td>
                    <td className={`px-3 py-2 text-right tabular-nums font-medium ${p.stock === 0 ? 'text-rose-600' : p.stock <= p.min_stock ? 'text-amber-600' : 'text-slate-700 dark:text-slate-200'}`}>
                      {p.stock ?? 0}
                    </td>
                      ...
                      {isAdmin ? (
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            onClick={() => save(p)}
                            disabled={busy || !dirty}
                            className="px-2 py-1 rounded-lg text-xs bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 hover:bg-blue-100 font-medium disabled:opacity-40 disabled:cursor-not-allowed"
